...
export const renderHtml = (itemList) => {
    if (!itemList) {
        return ""
    }
    let html = [];
    for (const item of itemList) {
        html.push(renderElement(item))
    }
    return html.join("")
...
export const renderElement = (item) => {
    ...
    if (id == 'text') {
        ...
        let childHtml = `<${tag}${renderProps(item)}>${renderSlot(item)}${renderHtml(item.__children__)}</${tag}>`;
        if (layout == 'formItem') {
...
export const renderProps = (item) => {
    ...
    if (item.__vModel__) {
        str.push(` v-model='${item.__vModel__}'`)
    }
    return str.join("");
...
const tags = {
    'el-button': (el) => {
        ...
        const loading = el.circle ? 'loading' : ''
        const loadingIcon = el['loading-icon'] ? `el['loading-icon']="${el['loading-icon']}"` : ''
        const color = el.color ? `color="${el.color}"` : ''
        let child = buildElButtonChild(el)

        if (child) child = `\n${child}\n` // 换行
        return `<${tag} ${type} ${text} ${icon} ${round} ${size} ${plain} ${disabled} ${circle} ${loading} ${loadingIcon} ${color}>${child}</${tag}>`
    },
    'el-input': (el) => {
 ...
 */
export function makeUpHtml(designConfig, type) {
    const htmlList = []
    confGlobal = designConfig
    // 判断布局是否都沾满了24个栅格，以备后续简化代码结构
    someSpanIsNot24 = designConfig.fields.some(
    ...
    // 将组件代码放进form标签
    let temp = buildFormTemplate(designConfig, htmlStr, type)
    // dialog标签包裹代码
    if (type === 'dialog') {
